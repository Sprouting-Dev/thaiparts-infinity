...
// Contact single-type populate (keep consistent with hero structure used on home)
// Only include what has nested media/components to avoid over-fetching.
// If you later add media inside contactInfo/contactForm/map, append populate rules.
export const contactPopulate = [
  // Contact hero uses the dedicated component sections.contact-hero (no CTAs, just title/subtitle/background)
  'populate[hero][populate][background]=1',
  'populate[hero][populate][title][populate][desktop]=1',
  'populate[hero][populate][title][populate][mobile][populate][lines]=1',
  // Page SEO
  'populate[seo][populate][ogImage]=1',
  // contactInfo / contactForm / map are flat components (only primitives), no populate needed right now
].join('&');